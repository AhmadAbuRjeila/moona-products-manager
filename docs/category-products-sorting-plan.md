...
3) **Service design**: Add a dedicated service that reuses `ProductsExportService` fetching + `ExcelWriter` export; expose method(s) that accept a category and rating range. **Status:** done
...
- Introduce `CategoryProductsRatingService` (name TBD) to orchestrate: fetch category products via `ProductsExportService.fetchAllProducts(createdAfter, updatedBefore, categoryId)`, apply sorting/rating logic, then call `ExcelWriter.exportProducts` for Excel output.
- Expose method `exportCategoryWithRatings(String categoryId, double minRating, double maxRating, String createdAfterIso, String updatedBeforeIso)` returning `Mono<String>` that writes the file and reports count; overloads for defaults.
- Keep rating logic isolated in helper (e.g., `CategoryRatingAssigner`) to unit test separately; input: `List<Product>`, rating bounds; output: sorted list with set ratings.
- Use a dedicated runner (new class) for rating + sorting export; do not reuse `ProductsExportRunner` to avoid overloading existing behavior.
- Avoid schema changes: stick to existing `Product` model and `ExcelWriter.exportProducts` to keep the export format identical.
- Future-proof: allow passing `createdAfter/updatedBefore` and optional default bounds from properties; log when bounds are insufficient for unique ratings.
